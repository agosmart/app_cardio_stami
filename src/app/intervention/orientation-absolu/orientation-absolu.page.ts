...
@Component({
  selector: "app-orientation-absolu",
  templateUrl: "./orientation-absolu.page.html",
  styleUrls: ["./orientation-absolu.page.scss"]
})
export class OrientationAbsoluPage implements OnInit {
  idUser: number;
  idEtab: number;
  dossierId: number;
  lastMotifId: number;
  ...
  ngOnInit() {
    ...
    this.activatedroute.paramMap.subscribe(paramMap => {
      ...
      if (!paramMap.has("dataPatientObj")) {
        ...
        if (this.dataPatient.stepId !== 25) {
          this.srvApp.stepUpdatePage(this.dossierId, 25, 8, this.token);
        }
        if (this.demandeAvisId > 0 && this.lastMotifId === 4) {
